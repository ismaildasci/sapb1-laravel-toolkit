...
/**
 * Inventory General Exit DTO (Goods Issue).
 *
 * @phpstan-consistent-constructor
 */
final class InventoryGenExitDto extends BaseDto
{
    /**
     * @param  array<InventoryGenExitLineDto>  $documentLines
     */
    ...
    protected static function mapFromArray(array $data): array
    {
        ...
        if (isset($data['DocumentLines']) && is_array($data['DocumentLines'])) {
            foreach ($data['DocumentLines'] as $line) {
                $lines[] = InventoryGenExitLineDto::fromArray($line);
            }
        }
    ...
    public function toArray(): array
    {
        ...
        if (! empty($this->documentLines)) {
            $data['DocumentLines'] = array_map(
                fn (InventoryGenExitLineDto $line) => $line->toArray(),
                $this->documentLines
            );
        }
